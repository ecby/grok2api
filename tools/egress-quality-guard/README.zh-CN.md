...
- 每个受测节点都需要绑定一个可调度、且能使用探测模型的 Grok Build 账号。一账号绑一节点时，探测不会借用其他节点的账号。
...
3. grok2api 只使用明确绑定到该节点的账号，并强制实际请求走被测节点，再发送固定流式 Prompt。若该节点没有可调度账号，探测返回 `egressQualityProbeNoAccount`，守护程序按无账号策略退避，不会借用其他节点的账号。即使其他 Provider 暴露同名模型，后端也会把探测路由固定为 Grok Build。
...
仅发生在质量探测中的模糊 403 不会冷却该节点绑定账号；明确的凭据失效、账号封禁和额度信号仍按原有规则处理。
...
账号调度阶段，例如该节点绑定账号当前不可调度，后端会返回独立错误码；守护程序按
