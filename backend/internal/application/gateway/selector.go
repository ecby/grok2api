...
// AcquireForKeyOnEgressNode is reserved for administrator probes. It only
// selects a credential bound to the requested node. The request layer still
// forces the physical call through nodeID so a disabled or cooling node can
// be tested without changing that account binding.
